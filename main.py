...
# Rich diagnostics terminal components
try:
...
log = logging.getLogger("bunny-bot")
...
class PanelPrinter:
    """Helper class to print highly styled, colorful DevOps terminal blocks."""
    @staticmethod
...
[bold #c8b6ff]//   ╚══════╝  ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═══╝   ╚═╝                      //[/]
[bold #c8b6ff]//                                                                    //[/]
[bold #c8b6ff]//         🐰  ///// B U N N Y   B O T   S Y S T E M /////  🐰         //[/]
[bold #c8b6ff]////////////////////////////////////////////////////////////////////////[/]
"""
        c.print(banner)
    ...
    def print_command_route(cmd_name: str, username: str, user_id: int, guild_name: str, latency: str, status: str = "SUCCESS"):
        ...
        c.print(f"[bold #c8b6ff]///// BUNNY BOT SYSTEM [ #bca6ff]COMMAND ROUTE[/] ///////[/]")
    ...
    def print_exception_box(error_type: str, message: str, file_name: str, line_num: str, method: str, raw_traceback: str):
        ...
        # Determine likely crash cause and dynamic suggestions based on error codes
        ...
        if "database is locked" in message.lower() or "sqlite" in error_type.lower():
            fix_guide = "Another task holds an exclusive write lock. Integrated SQLite WAL handlers are automatically queuing background retries."
        ...
        elif "accepted_terms" in message.lower() or "AttributeError" in error_type:
            fix_guide = "Type-checking mismatch. Checked variable returned as NoneType. Safely cast Row objects to dictionaries before retrieving values."

        c.print(f"[bold #ff5e7e]////////////////////////////////////////////////////////////////////////[/]")
        c.print(f"[bold #ff5e7e]// ❌ ENGINE EXCEPTION CAUGHT!                                        //[/]")
        c.print(f"[bold #ff5e7e]////////////////////////////////////////////////////////////////////////[/]")
...
class BunnyBot(commands.Bot):
    ...
    async def setup_hook(self):
        # Print glorious startup logo header
        PanelPrinter.print_startup_banner()

        # Initialise database WAL settings and tables
        db.init_db()
        log.info("Database WAL mode and schema initialized.")

        # Gather every cog inside directory dynamically
        cogs_found = []
        ...
        # Synchronize commands tree with Discord Gateway
        try:
    ...
    async def on_ready(self):
        ...
        # Beautiful startup dashboard panel
        try:
            console = Console()

            # System Specifications Sub-Panel
            ...
            # Database Integrations Sub-Panel
            ...
            db_table.add_row("Circulating Coins", f"{total_coins:,} coins 🪙")
            ...
            top_table.add_column(ratio=5)
            top_table.add_row(
                Panel(engine_table, title="[bold #c8b6ff]🐰 Bunny Bot Engine System[/]", box=ROUNDED, style="border #1c1c1f"),
            ...
            log.info(f"Bot connected as {self.user} (Latency: {ping}). Serving {len(self.guilds)} guilds.")
            log.warning(f"Failed to generate rich diagnostics layout: {e}")

    # ── Verify terms of service agreement ──────────────────────────────────────
    async def on_interaction(self, interaction: discord.Interaction):
        # We only gate slash commands
        if interaction.type == discord.InteractionType.application_command:
            ...
            # If user has not accepted terms, show the terms prompt view
            if not accepted:
                class TermsView(discord.ui.View):
                    def __init__(self):
                        super().__init__(timeout=None)

                    # Corrected: Button style uses root-level discord.ButtonStyle namespaces
                    ...
                    async def accept_terms(self, button_interaction: discord.Interaction, button: discord.ui.Button):
                        ...
                        try:
                            # Create user profile first if missing before updating terms state
                            ...
                            await button_interaction.response.send_message(
                                "Thank you! You have accepted our Terms of Service. You can now use all commands.", 
                                ephemeral=True
                            )
                            try:
                ...
                # Retrieve custom policies file if available
                terms_text = "Please accept our Terms and Privacy Policy before continuing."
                ...
                try:
                    ...
                    return  # Terminate slash command execution pathway
                except Exception as e:
                    log.error(f"Failed to send terms prompt: {e}")

        # Safely hand over interactions to command router
        try:
    ...
    async def on_application_command_error(
        ...
        # Extract full traceback lines to dissect inner errors (like sqlite locks)
        ...
        # Walk traceback to find the exact file and line number where the code crashed
        file_name = "main.py"
        line_num = "Unknown"
        ...
        # Print the styled exception box in the console
        ...
        # Let the user know on Discord in a friendly way
        em = discord.Embed(
...
if __name__ == "__main__":
    BunnyBot().run(TOKEN, log_handler=None)
